...
Note: gemini-embedding-001 is our newest text embedding model available in the Gemini API and Vertex AI.
...
For enterprise-grade applications and high-volume workloads, we suggest using embedding models on Vertex AI.
...
Caution: These models will soon be deprecated, so we strongly recommend developers to use the newest and stable Gemini embedding model instead.
