...
export class HowToCookMCPScraper {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      console.log('🔌 Connected to HowToCook MCP server');
    } catch (error) {
      console.error('❌ Failed to connect to HowToCook MCP server:', error);
  ...
  public async disconnect(): Promise<void> {
    if (this.client && this.transport) {
      ...
      this.transport = null;
      this.isConnected = false;
      console.log('🔌 Disconnected from HowToCook MCP server');
    }
  }
  ...
  public async searchRecipes(options: HowToCookSearchOptions): Promise<Recipe[]> {
    try {
      ...
      console.error('❌ HowToCook MCP search error:', error);
      return [];
    }
  }
  ...
  public async getRandomRecipe(partySize: number = 2): Promise<Recipe | null> {
    try {
      ...
      console.error('❌ HowToCook MCP random recipe error:', error);
      return null;
    }
  }
  ...
  public async getSmartMealPlan(
    ...
    try {
      ...
      const result = await this.client.callTool({
        ...
        arguments: {
          avoidItems: dietaryRestrictions,
          allergies: allergies,
          peopleCount: diners,
        },
      }) as MCPToolResult;
      ...
      console.error('❌ HowToCook MCP meal plan error:', error);
      return [];
    }
  }
