...
export class RecipeValidator {
     ...
     */
    static async validateAndFixRecipe(recipe, sourceUrl) {
     ...
     */
    static async validateBasicInfo(recipe, result, sourceUrl) {
     ...
     */
    static async validateTiming(recipe, result) {
     ...
     */
    static async validateIngredients(recipe, result) {
     ...
     */
    static async validateInstructions(recipe, result) {
        ...
        recipe.instructions.forEach((instruction, index) => {
            if (typeof instruction === 'string') {
                ...
                if (!instruction.timer_min)
                    instruction.timer_min = [];
                if (!instruction.equipment)
                    instruction.equipment = [];
                if (!instruction.mentioned_ingredients)
                    instruction.mentioned_ingredients = [];
            }
        });
     ...
     */
    static async validateMetadata(recipe, result) {
        // Servings validation
        if (!recipe.servings) {
     ...
     */
    static calculateCompletenessScore(recipe) {
            ...
            if (recipe[field] &&
                (Array.isArray(recipe[field]) ?
                    ...
                    recipe[field] !== null &&
                        ...
                        String(recipe[field]).trim().length > 0)) {
    ...
    static estimatePrepTime(instructions) {
        ...
        instructions.forEach(instruction => {
            ...
            if (lowerText.includes('mix') || lowerText.includes('combine') || lowerText.includes('stir'))
                prepTime += 2;
            if (lowerText.includes('wash') || lowerText.includes('rinse') || lowerText.includes('clean'))
                prepTime += 2;
            if (lowerText.includes('peel') || lowerText.includes('trim'))
                prepTime += 3;
            if (lowerText.includes('slice') || lowerText.includes('cut'))
                prepTime += 3;
        });
     ...
     */
    static estimateCookTime(instructions) {
        let cookTime = 0;
        ...
        instructions.forEach(instruction => {
            ...
            if (!hasTimingInfo) {
                if (lowerText.includes('bake') || lowerText.includes('roast'))
                    cookTime += 30;
                else if (lowerText.includes('simmer') || lowerText.includes('boil'))
                    cookTime += 15;
                else if (lowerText.includes('fry') || lowerText.includes('saute'))
                    cookTime += 8;
                else if (lowerText.includes('grill'))
                    cookTime += 10;
            }
        });
