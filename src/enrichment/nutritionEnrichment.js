...
async function getNutritionForIngredient(cleanName, grams) {
    if (!usdaApiKey)
        return null;
    try {
...
export async function getNutritionEnrichment(recipe) {
    if (!usdaApiKey || !recipe.ingredients)
        return {};
    const totals = {
    ...
    const processedIngredients = await Promise.all(recipe.ingredients.map(ingredientText => processIngredient({ name: ingredientText })));
    ...
    return { nutrition: finalNutrition, ingredients: processedIngredients.filter((i) => i !== null) };
}
