 ...
 */
function extractCookingAction(instructionText) {
    ...
    try {
        ...
        if (result.primary.length > 0) {
            ...
            if (result.errors) {
                result.errors.forEach(error => DataQualityLogger.getInstance().logIssue(instructionText, 'instruction-parser', `Error extracting cooking action: ${error}`));
            }
            return action;
        }
        ...
        DataQualityLogger.getInstance().logIssue('instructionParser', 'extractCookingAction', `Error extracting cooking action: ${error.message}`, { instructionText });
        return 'cook';
    }
 ...
 */
function extractTimerInfo(instructionText) {
    const timers = [];
 ...
 */
function extractKitchenEquipment(instructionText) {
    ...
    try {
        ...
        if (result.errors) {
            result.errors.forEach(error => DataQualityLogger.getInstance().logIssue('instructionParser', 'extractKitchenEquipment', error, { instructionText }));
        }
        return result.primary;
    }
    catch (error) {
        DataQualityLogger.getInstance().logIssue('instructionParser', 'extractKitchenEquipment', `Error extracting equipment: ${error.message}`, { instructionText });
        return [];
    }
}
/*
 ...
 */
function generateIngredientVariations(ingredientName) {
    const variations = [];
 ...
 */
function extractMentionedIngredients(instructionText, availableIngredients) {
    ...
    try {
        ...
        if (result.errors) {
            result.errors.forEach(error => DataQualityLogger.getInstance().logIssue('instructionParser', 'extractMentionedIngredients', error, { instructionText }));
        }
        return result.primary;
    }
    catch (error) {
        DataQualityLogger.getInstance().logIssue('instructionParser', 'extractMentionedIngredients', `Error parsing ingredients: ${error.message}`, { instructionText });
        return [];
    }
}
/*
 ...
 */
export function processInstruction(instructionText, stepNumber, availableIngredients = []) {
    try {
        ...
        const equipment = extractKitchenEquipment(text);
        ...
        const finalAction = validActions.includes(action) ? action : null;
 ...
 */
export function processInstructions(instructions, availableIngredients = []) {
    return instructions.map((instruction, index) => processInstruction(instruction, index + 1, availableIngredients));
}
