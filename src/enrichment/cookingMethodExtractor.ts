...
export class CookingMethodExtractor {
  ...
  private static readonly ACTION_TIME_WEIGHTS = {
   ...
   */
  static extractCookingMethod(instructions: InstructionStep[]): string {
    if (!instructions || instructions.length === 0) return 'No Method';
      ...
      for (const [method, keywords] of Object.entries(this.COOKING_METHODS)) {
        for (const keyword of keywords) {
          if (text.includes(keyword)) {
            const actionWeight = this.ACTION_TIME_WEIGHTS[method as keyof typeof this.ACTION_TIME_WEIGHTS] || 5;
    ...
    if (Object.keys(methodScores).length === 0) return 'Mixed';
   ...
   */
  private static calculateStepWeight(instruction: InstructionStep, index: number, totalSteps: number): number {
    let weight = 1;
   ...
   */
  static extractDetailedMethods(instructions: InstructionStep[]): string[] {
