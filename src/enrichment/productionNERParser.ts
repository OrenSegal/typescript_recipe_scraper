import { IngredientCategory, RecipeIngredient } from '../types.js';
...
export class ProductionNERParser {
   ...
   */
  static async parseIngredient(text: string): Promise<RecipeIngredient> {
    ...
    try {
      ...
      const result: RecipeIngredient = {
        text,
        name,
        order_index: 0, // Will be set properly when integrated with recipe processing
   ...
   */
  static async parseIngredients(ingredients: string[]): Promise<RecipeIngredient[]> {
   ...
   */
  private static extractQuantity(text: string): number | number[] | null {
   ...
   */
  private static categorizeIngredient(name: string): typeof IngredientCategory._type | null {
    const lowerName = name.toLowerCase();
    
    if (/\b(beef|pork|lamb|veal|ground meat)\b/i.test(lowerName)) return 'Meat';
    if (/\b(chicken|turkey|duck|poultry)\b/i.test(lowerName)) return 'Poultry';
    if (/\b(fish|salmon|tuna|shrimp|crab|seafood)\b/i.test(lowerName)) return 'Seafood';
    if (/\b(milk|cheese|butter|cream|yogurt|dairy)\b/i.test(lowerName)) return 'Dairy';
    if (/\b(onion|garlic|tomato|carrot|celery|spinach|lettuce|cucumber|pepper|vegetable)\b/i.test(lowerName)) return 'Vegetables';
    if (/\b(apple|banana|orange|lemon|lime|berry|fruit)\b/i.test(lowerName)) return 'Fruits';
    if (/\b(rice|wheat|oats|quinoa|grain|flour|bread|pasta)\b/i.test(lowerName)) return 'Grains';
    if (/\b(parsley|basil|oregano|thyme|rosemary|herb|cilantro)\b/i.test(lowerName)) return 'Herbs & Spices';
    if (/\b(salt|pepper|cumin|paprika|ginger|spice|cinnamon)\b/i.test(lowerName)) return 'Herbs & Spices';
    if (/\b(oil|vinegar|sugar|vanilla|baking)\b/i.test(lowerName)) return 'Pantry Staples';
   ...
   */
  private static extractNotes(text: string): string | null {
    const prepMethods = [];
    const lowerText = text.toLowerCase();
    
    if (lowerText.includes('chopped')) prepMethods.push('chopped');
    if (lowerText.includes('diced')) prepMethods.push('diced');
    if (lowerText.includes('sliced')) prepMethods.push('sliced');
    if (lowerText.includes('minced')) prepMethods.push('minced');
    if (lowerText.includes('grated')) prepMethods.push('grated');
    if (lowerText.includes('fresh')) prepMethods.push('fresh');
    if (lowerText.includes('dried')) prepMethods.push('dried');
    if (lowerText.includes('cooked')) prepMethods.push('cooked');
    if (lowerText.includes('optional')) prepMethods.push('optional');
    if (lowerText.includes('if needed')) prepMethods.push('if needed');
    if (lowerText.includes('as needed')) prepMethods.push('as needed');
   ...
   */
  private static createFallbackIngredient(text: string): RecipeIngredient {
    return {
      text,
      name: text,
      order_index: 0,
   ...
   */
  private static recordSuccess(startTime: number): void {
   ...
   */
  private static calculateGramsSimple(quantity: number | number[] | null, unit: string | null, ingredient: string): number | null {
