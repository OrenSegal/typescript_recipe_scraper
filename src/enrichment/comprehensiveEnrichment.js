...
export class ComprehensiveEnrichment {
    ...
    static async enrichRecipe(rawRecipeData, options = {}) {
        try {
            ...
            const recipeData = rawRecipeData;
            let inferredCuisineType = recipeData.cuisines || [];
            let inferredTags = recipeData.tags || [];
            let inferredDifficultyLevel = recipeData.effort_level || 3;
            let inferredDietaryRestrictions = recipeData.dietary_restrictions || [];
            ...
            if (!recipeData.effort_level) {
                inferredDifficultyLevel = ComprehensiveEnrichment.inferDifficultyLevel(enrichedInstructions, enrichedIngredients);
            }
            ...
            const finalRecipeData = {
                ...
                tags: inferredTags || [],
                effort_level: inferredDifficultyLevel,
            ...
            const detectedMealTypes = detectMealTypes(rawRecipeData.title, rawRecipeData.description || '', enrichedIngredients, enrichedInstructions, prepTime || 0, cookTime || 0);
            ...
            try {
                ...
                if (nutritionResult.ingredients) {
                    nutritionEnrichedIngredients = nutritionResult.ingredients;
                }
            }
            catch (error) {
            ...
            try {
                const tempRecipeForEmbedding = {
                    ...
                    effort_level: finalRecipeData.effort_level,
            ...
            const qualityMetrics = ComprehensiveEnrichment.calculateQualityMetrics(nutritionEnrichedIngredients, enrichedInstructions, finalRecipeData);
            ...
            const enrichedRecipe = {
                ...
                nutrition_data_source: nutritionData ? 'api' : 'estimated',
     ...
     */
    static async enrichIngredients(ingredients) {
        const enriched = [];
     ...
     */
    static async enrichInstructions(instructions, availableIngredients = []) {
        try {
            ...
            const ingredientReferences = availableIngredients.map(ing => ({
            ...
            const atomicSteps = AtomicInstructionParser.processMultipleInstructions(instructions, ingredientReferences);
            // Convert atomic steps to EnrichedInstruction format
            const enriched = atomicSteps.map(step => {
     ...
     */
    static extractComprehensiveMetadata(rawRecipeData) {
        return {
            cuisineTypes: [],
            dietaryRestrictions: [],
     ...
     */
    static parseIngredientFallback(ingredient) {
        ...
        return {
            clean_name: parts.slice(-2).join(' '), // Last two words as name
     ...
     */
    static estimatePrepTimeFromInstructions(instructions) {
        let prepTime = 0;
     ...
     */
    static estimateCookTimeFromInstructions(instructions) {
        let cookTime = 0;
        instructions.forEach(inst => {
            const text = inst.text.toLowerCase();
     ...
     */
    static inferCuisineFromIngredients(ingredients) {
        const ingredientNames = ingredients.map(i => i.name.toLowerCase() || i.name.toLowerCase());
        // Italian cuisine indicators
        if (ingredientNames.some(name => ['pasta', 'parmesan', 'mozzarella', 'basil', 'oregano', 'tomato'].some(italian => name.includes(italian)))) {
            return ['Italian'];
        }
        // Asian cuisine indicators
        if (ingredientNames.some(name => ['soy sauce', 'ginger', 'garlic', 'sesame', 'rice', 'noodles'].some(asian => name.includes(asian)))) {
            return ['Asian'];
        }
        // Mexican cuisine indicators
        if (ingredientNames.some(name => ['cumin', 'chili', 'cilantro', 'lime', 'avocado', 'beans'].some(mexican => name.includes(mexican)))) {
            return ['Mexican'];
        }
     ...
     */
    static generateTagsFromRecipe(title, description, ingredients) {
        const tags = [];
        ...
        if (text.includes('grill'))
            tags.push('grilled');
        if (text.includes('fry'))
            tags.push('fried');
        if (text.includes('no-cook') || text.includes('raw'))
            tags.push('no-cook');
        // Add ingredient-based tags
        if (ingredients.some(i => i.category === 'Vegetables'))
            tags.push('vegetable-based');
        if (ingredients.some(i => i.category === 'Meat'))
            tags.push('meat');
     ...
     */
    static inferDifficultyLevel(instructions, ingredients) {
     ...
     */
    static analyzeDietaryRestrictions(ingredients) {
        const restrictions = [];
        const ingredientNames = ingredients.map(i => i.name.toLowerCase() || i.name.toLowerCase());
        // Check for common dietary restrictions
        const hasMeat = ingredientNames.some(name => ['chicken', 'beef', 'pork', 'lamb', 'turkey', 'fish', 'salmon', 'tuna'].some(meat => name.includes(meat)));
        const hasDairy = ingredientNames.some(name => ['milk', 'cheese', 'butter', 'cream', 'yogurt'].some(dairy => name.includes(dairy)));
        const hasGluten = ingredientNames.some(name => ['flour', 'wheat', 'bread', 'pasta'].some(gluten => name.includes(gluten)));
        if (!hasMeat)
     ...
     */
    static calculateQualityMetrics(ingredients, instructions, recipeData) {
        ...
        if (ingredients.length >= 3)
            completenessScore++; // At least 3 ingredients
     ...
     */
    static calculateTotalTime(prepTime, cookTime) {
     ...
     */
    static extractAuthor(author, sourceUrl) {
        if (author)
            return author;
    ...
    static extractCookingMethod(instruction) {
        // Handle both string and object formats
        const instructionText = typeof instruction === 'string' ? instruction : instruction.text || '';
        const text = instructionText.toLowerCase();
     ...
     */
    static inferCookingMethod(instructions) {
     ...
     */
    static calculateBasicHealthScore(ingredients, nutrition) {
        let healthScore = 50; // Base score
        // Positive points for healthy ingredients
        const healthyCategories = ['Vegetables', 'Fruits', 'Herbs & Spices', 'Legumes & Pulses'];
        const healthyCount = ingredients.filter(ing => healthyCategories.includes(ing.category || '')).length;
        ...
        const unhealthyCount = ingredients.filter(ing => unhealthyCategories.includes(ing.category || '')).length;
        healthScore -= Math.min(unhealthyCount * 3, 15); // Max -15 points
        // Nutrition-based scoring
        if (nutrition) {
            // Positive for high fiber, protein
            if (nutrition.fiber_g && nutrition.fiber_g > 5)
                healthScore += 10;
            if (nutrition.protein_g && nutrition.protein_g > 15)
                healthScore += 10;
            // Negative for high sodium, saturated fat
            if (nutrition.sodium_mg && nutrition.sodium_mg > 1000)
                healthScore -= 10;
            if (nutrition.saturated_fat_g && nutrition.saturated_fat_g > 10)
                healthScore -= 10;
        }
