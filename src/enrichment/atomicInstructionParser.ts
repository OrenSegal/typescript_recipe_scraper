...
import { InstructionStep, RecipeIngredient } from '../types.js';
import { CookingAction as CookingActionType, KitchenEquipment as KitchenEquipmentType, CookingActionSchema } from '../../recipe-parsing-specification.js';
...
export class AtomicInstructionParser {
  ...
  private static cookingActions: Set<string> = new Set();
  private static kitchenEquipment: Set<string> = new Set();
  private static sentenceBreakers = /[.!?]+(?:\s|$)/g;
   ...
   */
  static breakIntoAtomicSteps(instructionText: string): string[] {
   ...
   */
  private static splitByCoordinatingConjunctions(sentence: string): string[] {
   ...
   */
  private static hasAction(text: string): boolean {
   ...
   */
  static extractMainAction(stepText: string): CookingActionType | null {
    const doc = nlp(stepText.toLowerCase());
    const verbs = doc.verbs().out('array');
    
    // Use Zod schema for robust validation instead of hardcoded casting
    const isValidCookingAction = (action: string): action is CookingActionType => {
   ...
   */
  static extractMentionedIngredients(
    stepText: string, 
    ...
    const ingredientDict = new Map<string, string>();
    
    for (const ingredient of availableIngredients) {
      // Use type guard to ensure ingredient has valid name field
      const isValidIngredient = (ing: unknown): ing is { name: string } => {
        return typeof ing === 'object' && ing !== null && 
               'name' in ing && typeof (ing as any).name === 'string' && 
               (ing as any).name.length > 0;
      };
   ...
   */
  static extractMentionedEquipment(stepText: string): KitchenEquipmentType[] {
    const mentionedEquipment: KitchenEquipmentType[] = [];
    const lowerStepText = stepText.toLowerCase();
    ...
    for (const candidate of candidates) {
      ...
      if (equipmentMatcher.isKitchenEquipment(lowerCandidate)) {
        ...
        const canonicalName = equipmentMatcher.getCanonicalName(lowerCandidate) as KitchenEquipmentType;
    ...
    try {
      ...
      for (const candidate of allCandidates) {
        if (candidate.length > 2) {
          ...
          if (equipmentMatch) {
            const canonicalName = equipmentMatch.canonical_name as KitchenEquipmentType;
            if (!mentionedEquipment.includes(canonicalName)) {
              mentionedEquipment.push(canonicalName);
            }
          }
   ...
   */
  static extractTimerInfo(stepText: string): number[] {
    const timers: number[] = [];
   ...
   */
  static processCompoundInstruction(
   ...
   */
  static processMultipleInstructions(
    instructions: string[],
    availableIngredients: RecipeIngredient[] = []
  ): InstructionStep[] {
    let currentStepNumber = 1;
    const allSteps: InstructionStep[] = [];
    
    for (const instruction of instructions) {
      const atomicSteps = this.processCompoundInstruction(
        instruction,
        currentStepNumber,
