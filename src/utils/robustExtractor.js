...
export class RobustExtractor {
     ...
     */
    static extractCookingActions(text) {
        try {
            // Method 1: NLP-based verb extraction
            const doc = nlp(text.toLowerCase());
            const verbs = doc.verbs().out('array');
            const nlpActions = verbs.filter((verb) => COOKING_ACTIONS.some((action) => verb.includes(action) || action.includes(verb) ||
            ...
            // Default fallback
            DataQualityLogger.getInstance().logIssue('RobustExtractor', 'extractCookingActions', 'No cooking action found', { text });
            return {
                primary: ['cook'],
                confidence: 0.3,
                method: 'hardcoded_fallback',
            ...
            DataQualityLogger.getInstance().logIssue('RobustExtractor', 'extractCookingActions', `Error in extractCookingActions: ${error.message}`, { text });
            return {
                primary: ['cook'],
                confidence: 0.1,
                method: 'hardcoded_fallback',
     ...
     */
    static extractKitchenEquipment(text) {
        try {
            // Method 1: NLP-based noun extraction
            const doc = nlp(text.toLowerCase());
            const nouns = doc.nouns().out('array');
            const nlpEquipment = nouns.filter((noun) => KITCHEN_EQUIPMENT.some((equipment) => noun.includes(equipment) ||
            ...
            DataQualityLogger.getInstance().logIssue('RobustExtractor', 'extractKitchenEquipment', `Error in extractKitchenEquipment: ${error.message}`, { text });
            return {
                primary: [],
                confidence: 0.1,
                method: 'hardcoded_fallback',
     ...
     */
    static extractMentionedIngredients(text, availableIngredients = []) {
        try {
            const mentioned = [];
            ...
            const doc = nlp(text.toLowerCase());
            const nouns = doc.nouns().out('array');
            const nlpIngredients = nouns.filter((noun) => COMMON_INGREDIENTS.some((ingredient) => noun.includes(ingredient) || ingredient.includes(noun) ||
            ...
            DataQualityLogger.getInstance().logIssue('RobustExtractor', 'extractMentionedIngredients', `Error in extractMentionedIngredients: ${error.message}`, { text });
            return {
                primary: [],
                confidence: 0.1,
                method: 'hardcoded_fallback',
