...
const WebsiteSchema = z.object({
  ...
  'Sub-sitemap URLs': z.string().optional()
});
...
export class WebsiteManager {
    ...
    try {
      ...
      for (const record of records) {
        try {
          ...
          // Create website object mapping to the canonical Website type
          const website: Website = {
            id: this.websites.length + 1,
            name: validatedRecord['Website Name'],
            base_url: validatedRecord['Main URL'],
            sitemap_url: validatedRecord['Main Sitemap URL'],
   ...
   */
  private calculatePriority(website: z.infer<typeof WebsiteSchema>): number {
   ...
   */
  getHighPriorityWebsites(minPriority: number = 8): Website[] {
