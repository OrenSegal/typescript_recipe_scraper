...
export class RecipeProcessor {
  [x: string]: any;
  private static instance: RecipeProcessor;
   ...
   */
  public static getInstance(): RecipeProcessor {
  ...
  public async processRecipe(
    ...
    include_nutrition: boolean = true,
    generate_embedding: boolean = true
   ...
   */
  private mapToRecipeSchema(enrichedRecipe: any, rawRecipe: RawScrapedRecipe): Recipe {
    return {
   ...
   */
  private calculateCompletenessScore(recipe: Recipe): number {
    let score = 0;
    const maxScore = 100;
    
    // Basic fields (40 points)
    if (recipe.title) score += 10;
    if (recipe.description) score += 10;
    if (recipe.ingredients.length > 0) score += 10;
    if (recipe.instructions.length > 0) score += 10;
    
    // Timing information (20 points)
    if (recipe.prep_time_minutes) score += 10;
    if (recipe.cook_time_minutes) score += 10;
    
    // Metadata (20 points)
    if (recipe.servings) score += 5;
    if (recipe.effort_level) score += 5;
    if (recipe.cuisines && recipe.cuisines.length > 0) score += 5;
    if (recipe.author) score += 5;
    
    // Additional details (20 points)
    if (recipe.image_url) score += 10;
    if (recipe.dietary_restrictions && recipe.dietary_restrictions.length > 0) score += 5;
    if (recipe.tags.length > 0) score += 5;
   ...
   */
  private calculateParsingConfidence(recipe: Recipe): number {
    let confidence = 50; // Base confidence
    
    // Ingredient parsing quality
    if (recipe.ingredients.length > 0) {
      const avgIngredientLength = recipe.ingredients.reduce((sum, ing) => 
        sum + (ing.name?.length || 0), 0) / recipe.ingredients.length;
      if (avgIngredientLength > 3) confidence += 15;
    }
    
    // Instruction parsing quality
    if (recipe.instructions.length > 0) {
      const avgInstructionLength = recipe.instructions.reduce((sum, inst) => 
        sum + (inst.text?.length || 0), 0) / recipe.instructions.length;
      if (avgInstructionLength > 20) confidence += 15;
    }
    
    // Metadata completeness
    if (recipe.servings && recipe.prep_time_minutes) confidence += 10;
    if (recipe.cuisines && recipe.cuisines.length > 0) confidence += 5;
    if (recipe.effort_level && recipe.effort_level !== 3) confidence += 5; // Non-default value
...
export class ProcessorFactory {
  public static createProcessor(): RecipeProcessor {
...
export class ProcessingUtils {
  public static validateProcessingOptions(options: ProcessingOptions): boolean {
  ...
  public static getDefaultOptions(): ProcessingOptions {
    return {
      include_ai: false,
